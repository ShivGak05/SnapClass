...
from src.database.db import check_user_exists
from src.database.db import create_teacher
import time
from src.database.db import teacher_login
from src.components.dialogue_create_subject import dialogue_create_subject
...
def teacher_screen():
    ...
    elif st.session_state['teacher_login']=="register":
...
def teacher_tab_manage_subjects():
    ...
    with col2:
        if st.button("Create new Subject",width='stretch'):
            dialogue_create_subject(teacher_id)
...
def teacher_screen_login():
    ...
    with col2:
        if st.button("Go back to Home",type='secondary',key='loginbackbtn',shortcut="control+backspace"):
            ...
            st.rerun()
    st.header("Login using your password")
    st.space()
    st.space()
...
def register_teacher(username,name,password,password_confirm):
    ...
    try:
        ...
        return False,"Error creating teacher profile"
